...
app=FastAPI(title='PHOTOSYNC API',version='0.9.0')
...
def add_web_history(client_id,kind,entry):
    ...
    tid=entry.get('transfer_id'); history=[x for x in history if not tid or x.get('transfer_id')!=tid]
    history=[entry]+history[:499]; write_json(web_history_path(client_id,kind),history)
...
def append_received_to_paired_web_clients(device_id,info):
    for d in WEB_CLIENTS_DIR.iterdir() if WEB_CLIENTS_DIR.exists() else []:
        ...
        entry=dict(info);entry['source']='phone';entry['received_from_device']=safe_device_id(device_id);entry['url']=f'/web-client/file/{cid}/{safe_device_id(device_id)}/app/{info["stored_filename"]}'
        add_web_history(cid,'received',entry)
...
def web_client_files(web_client_id:str,kind:str='sent'):
    if kind not in {'sent','received'}:raise HTTPException(400,'invalid kind');get_web_meta(web_client_id);return read_json(web_history_path(web_client_id,kind),[])
...
def web_client_file(web_client_id:str,device_id:str,source:str,filename:str):
    ...
    meta=get_web_meta(cid);allowed=False
