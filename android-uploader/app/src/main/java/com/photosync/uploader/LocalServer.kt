...
class LocalServer(private val context: Context, private val port: Int = 18000) {
    ...
    fun stop() {
        running = false
        sessions.clear(); failedAttempts.clear()
        try { serverSocket?.close() } catch (_: Exception) {}
        serverSocket = null
    }

    /** Generates a new app-owned PIN and invalidates every browser session. */
        ...
        socket.use {
            try {
                ...
                val response = when {
                    ...
                    path == "/api/session" -> if (authorized) {
                        val remaining = sessions[token]?.minus(System.currentTimeMillis()) ?: sessionLifetimeMs
                        responseJson("{\"authorized\":true,\"expires_in_seconds\":${(remaining / 1000).coerceAtLeast(0)} }")
                    ...
                    path.startsWith("/files/") && method == "GET" -> fileResponse(path, query["download"] == "1")
    ...
    private fun writeResponse(socket: Socket, response: Response) {
        ...
        val header = "HTTP/1.1 ${response.status}\r\n" +
            ...
            "Pragma: no-cache\r\n" +
            response.extraHeaders + cookie + "Connection: close\r\n\r\n"
    ...
    private fun pair(clientKey: String, supplied: String): Response {
        ...
        if (attempts.size >= maxAttemptsPerMinute) {
            return responseJson("{\"paired\":false,\"message\":\"Too many attempts; try again later\"}", "429 Too Many Requests")
        }
        if (supplied.length == 6 && supplied == pin) {
            ...
            return Response(
                ...
                setCookie = "photosync_session=$token; Max-Age=1800; Path=/; HttpOnly; SameSite=Lax"
            )
        }
        attempts.add(now)
    ...
    private fun logoutResponse() = Response(
        ...
        setCookie = "photosync_session=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax"
    )

    private fun parseCookie(header: String?, name: String): String? = header?.split(';')?.map { it.trim() }
    ...
    private fun fileJson(file: File, source: String): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("download_url", "/files/$source/${URLEncoder.encode(file.name, "UTF-8").replace("+", "%20")}?download=1")
    ...
    private fun filesResponse(source: String?): Response {
        ...
        dir.listFiles()?.filter { it.isFile }?.sortedByDescending { it.lastModified() }?.forEach {
            array.put(fileJson(it, if (dir == uploadsDir) "app" else "received"))
        }
        return responseJson(array.toString())
    }

    private fun fileResponse(path: String, download: Boolean): Response {
        ...
        return Response(
            "200 OK", contentType(name), bytes = file.readBytes(),
            extraHeaders = "Content-Disposition: $disposition; filename=\"${name.replace("\"", "_")}\"\r\n"
        )
    }
    ...
    private fun uploadResponse(input: InputStream, headers: Map<String, String>, query: Map<String, String>): Response {
        ...
        val length = headers["content-length"]?.toLongOrNull()
            ?: return responseJson("{\"detail\":\"Content-Length required\"}", "411 Length Required")
    ...
    private fun uniqueDestination(dir: File, original: String): File {
        ...
        var n = 1
        while (candidate.exists()) candidate = File(dir, "${System.currentTimeMillis()}__${n++}__$original")
        return candidate
    }
    ...
    private fun InputStream.copyExactlyTo(output: java.io.OutputStream, expected: Long) {
        output.use { out ->
            ...
            while (total < expected) {
                ...
                if (read <= 0) throw EOFException("Unexpected end of upload")
    ...
    private fun infoResponse(): Response {
        val files = (uploadsDir.listFiles()?.filter { it.isFile } ?: emptyList()) +
            (downloadsDir.listFiles()?.filter { it.isFile } ?: emptyList())
        val used = files.sumOf { it.length() }
        val total = rootDir.totalSpace
        val free = rootDir.usableSpace
        return responseJson(
            "{\"server\":\"photosync-android\",\"ip\":${json(localIpv4())}," +
                "\"port\":$port,\"pin_required\":true,\"running\":${isRunning()}," +
...
<div id='modal' class='modal hidden'><div class='top'><button id='zoomOut'>−</button><button id='zoomReset'>100%</button><button id='zoomIn'>+</button><a id='download' class='action' download>Download</a><button id='close'>Close</button></div><div id='viewer' class='viewer'></div></div>
<script>(()=>{const $=id=>document.getElementById(id);function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;')}function fmt(n){n=Number(n)||0;const u=['B','KB','MB','GB','TB'];let i=0;while(n>=1024&&i<u.length-1){n/=1024;i++}return n.toFixed(i?1:0)+' '+u[i]}async function pair(){const pin=$('pin').value.trim(),msg=$('msg'),button=$('pairButton');if(!/^\d{6}$/.test(pin)){msg.textContent='Enter a 6-digit PIN';return}button.disabled=true;msg.textContent='Connecting…';try{const r=await fetch('/api/pair?pin='+encodeURIComponent(pin),{method:'POST',credentials:'same-origin',cache:'no-store',headers:{Accept:'application/json'}});const d=await r.json().catch(()=>({}));if(!r.ok||d.paired!==true)throw Error(d.message||('Pairing failed (HTTP '+r.status+')'));msg.textContent='Connected ✓';show()}catch(e){msg.textContent='Connection failed: '+(e.message||'Network error')}finally{button.disabled=false}}async function session(){try{const r=await fetch('/api/session?ts='+Date.now(),{credentials:'same-origin',cache:'no-store'});if(r.ok){const d=await r.json();if(d.authorized)show()}}catch(_){}}function show(){$('gate').classList.add('hidden');$('app').classList.remove('hidden');load()}async function logout(){try{await fetch('/api/logout',{method:'POST',credentials:'same-origin',cache:'no-store'})}finally{location.reload()}}$('pairButton').onclick=pair;$('pin').onkeydown=e=>{if(e.key==='Enter')pair()};$('logoutButton').onclick=logout;$('picker').onchange=()=>{[...$('picker').files].forEach(upload);$('picker').value=''};function upload(file){const q=document.createElement('div');q.className='box';q.innerHTML='<b>'+esc(file.name)+'</b><div class=progress><i></i></div><span>0%</span> <button class=danger>Cancel</button>';const bar=q.querySelector('i'),label=q.querySelector('span'),cancel=q.querySelector('button');$('queue').prepend(q);const xhr=new XMLHttpRequest();xhr.open('POST','/upload?source=web&filename='+encodeURIComponent(file.name));xhr.upload.onprogress=e=>{if(e.lengthComputable){const p=Math.round(e.loaded/e.total*100);bar.style.width=p+'%';label.textContent=p+'% • '+fmt(e.loaded)+' / '+fmt(e.total)}};xhr.onload=()=>{if(xhr.status>=200&&xhr.status<300){label.textContent='Uploaded ✓';bar.style.width='100%';load()}else label.textContent='Failed: '+xhr.responseText};xhr.onerror=()=>label.textContent='Network error';xhr.onabort=()=>label.textContent='Cancelled';cancel.onclick=()=>xhr.abort();xhr.send(file)}async function getFiles(source){const r=await fetch('/files?source='+encodeURIComponent(source)+'&ts='+Date.now(),{cache:'no-store',credentials:'same-origin'});if(!r.ok)throw Error('HTTP '+r.status);return r.json()}async function load(){try{const [a,b,info]=await Promise.all([getFiles('app'),getFiles('received'),fetch('/api/info?ts='+Date.now(),{cache:'no-store',credentials:'same-origin'}).then(r=>r.json())]);$('sent').innerHTML=cards(a,'sent');$('received').innerHTML=cards(b,'received');$('stats').textContent='Used '+fmt(info.used_bytes)+' • Free '+fmt(info.free_bytes)+' • Total '+fmt(info.total_bytes);bind()}catch(e){if(String(e.message).includes('401'))location.reload()}}function openViewer(p){$('modal').classList.remove('hidden');const v=$('viewer');v.innerHTML='';let el;if(p.type==='image'){el=document.createElement('img');el.src=p.url;el.dataset.scale='1';el.style.transform='scale(1)'}else if(p.type==='video'){el=document.createElement('video');el.src=p.url;el.controls=true;el.autoplay=true}else{el=document.createElement('iframe');el.src=p.url;el.style='width:95vw;height:85vh;border:0;background:white'}v.appendChild(el);$('download').href=p.download_url||p.url+'?download=1';$('download').download=p.filename||'file';$('zoomIn').onclick=()=>{if(p.type==='image'){let s=Number(el.dataset.scale)+.25;el.dataset.scale=s;el.style.transform='scale('+s+')'}};$('zoomOut').onclick=()=>{if(p.type==='image'){let s=Math.max(.25,Number(el.dataset.scale)-.25);el.dataset.scale=s;el.style.transform='scale('+s+')'}};$('zoomReset').onclick=()=>{if(p.type==='image'){el.dataset.scale='1';el.style.transform='scale(1)'}}}$('close').onclick=()=>{$('modal').classList.add('hidden');$('viewer').innerHTML=''};function cards(a,bucket){window.__files[bucket]=a;if(!a.length)return '<div class=muted>None</div>';return a.map((p,i)=>{const u=esc(p.url),name=esc(p.filename||'file');let media=p.type==='image'?'<img class=thumb loading=lazy src="'+u+'">':p.type==='video'?'<video class=videoThumb muted preload=metadata src="'+u+'"></video>':'<div style="aspect-ratio:1;display:grid;place-items:center;font-size:42px">'+(p.type==='pdf'?'📕':'📄')+'</div>';return '<div class=card data-bucket="'+bucket+'" data-i="'+i+'">'+media+'<div class=meta>'+name+'<br><a class=action href="'+esc(p.download_url||p.url+'?download=1')+'" download>Download</a></div></div>'}).join('')}function bind(){document.querySelectorAll('.card').forEach(c=>{c.onclick=e=>{if(e.target.tagName==='A')return;const data=window.__files[c.dataset.bucket]||[];const p=data[Number(c.dataset.i)];if(p)openViewer(p)}})}window.__files={};session();setInterval(()=>{if(!$('app').classList.contains('hidden'))load()},3000)})();</script></body></html>"""
    }
