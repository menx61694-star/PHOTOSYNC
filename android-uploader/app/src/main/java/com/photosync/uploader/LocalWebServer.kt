...
class LocalWebServer(private val context: Context, private val port: Int) {
    ...
    private fun page(): String {
        ...
        return """<!doctype html><html><head><meta name=viewport content='width=device-width,initial-scale=1'><title>PHOTOSYNC</title><style>body{font-family:system-ui;background:#0b0f14;color:#fff;margin:0;padding:20px}main{max-width:950px;margin:auto}.box{background:#172235;border:1px solid #2b3950;border-radius:16px;padding:16px;margin-bottom:16px}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}button,.action{background:#bda4ff;border:0;border-radius:10px;padding:10px 16px;font-weight:700;cursor:pointer;color:#17131f;text-decoration:none}.danger{background:#44222a;color:#fff}.muted{color:#aab7ca}.hidden{display:none}.gate{max-width:430px;margin:12vh auto}.files{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.card{background:#111923;border-radius:12px;overflow:hidden}.thumb,.videoThumb{width:100%;aspect-ratio:1;object-fit:cover;display:block;cursor:pointer}.meta{padding:9px;font-size:13px}.modal{position:fixed;inset:0;background:rgba(0,0,0,.92);display:flex;flex-direction:column;z-index:10}.modal.hidden{display:none}.top{display:flex;gap:8px;padding:12px;justify-content:center}.viewer{flex:1;display:flex;align-items:center;justify-content:center;overflow:auto}.viewer img{max-width:none;max-height:85vh}.viewer video{max-width:95vw;max-height:85vh}</style></head><body><main><div id=gate class='box gate'><h1>PHOTOSYNC</h1><p>Android local server is running.</p><p class=muted>Web address: <code>$address</code></p><h2>Pair this browser</h2><p class=muted>Enter the 6-digit PIN shown in the PhotoSync app.</p><div class=row><input id=pin inputmode=numeric maxlength=6 placeholder=PIN><button id=pair>Connect</button></div><p id=msg class=muted></p></div><div id=app class=hidden><div class=box><div class=row style='justify-content:space-between'><h1>PHOTOSYNC Local Server</h1><button id=logout class=danger>Disconnect</button></div><div id=stats class=muted></div></div><div class=box><h2>Send files to this phone</h2><input id=pick type=file multiple></div><div class=box><h2>Send text to this phone</h2><textarea id=textInput rows=1 maxlength=1048576 placeholder="Type or paste text here..." style="width:100%;box-sizing:border-box;background:#111923;color:#fff;border:1px solid #2b3950;border-radius:10px;padding:10px;resize:none;min-height:42px;max-height:260px;overflow-y:auto"></textarea><div class=row style="margin-top:10px"><button id=pasteText>Paste</button><button id=sendText>Send</button><button id=receiveText>Receive</button><button id=clearText>Clear</button><span id=textMsg class=muted></span></div><div id=textReceived class=muted style="margin-top:10px"></div></div><div class=box><h2>Sent by app</h2><div id=sent class=files></div></div><div class=box><h2>Received from web</h2><div id=received class=files></div></div></div></main><div id=modal class='modal hidden'><div class=top><button id=zo>-</button><button id=zr>100%</button><button id=zi>+</button><a id=dl class=action download>Download</a><button id=cl>Close</button></div><div id=view class=viewer></div></div><script>const $=x=>document.getElementById(x);let files={};async function pair(){let p=$('pin').value.trim();if(!/^\d{6}$/.test(p)){$('msg').textContent='Enter a 6-digit PIN';return}let r=await fetch('/api/pair?pin='+p,{method:'POST',credentials:'same-origin',cache:'no-store'});let d=await r.json().catch(()=>({}));if(!r.ok){$('msg').textContent=d.message||'Pairing failed';return}$('gate').classList.add('hidden');$('app').classList.remove('hidden');load()}function showGate(message){$('app').classList.add('hidden');$('gate').classList.remove('hidden');$('msg').textContent=message||'Enter the current 6-digit PIN to connect'}async function load(){try{let rs=await Promise.all([fetch('/files?source=app&x='+Date.now(),{cache:'no-store'}),fetch('/files?source=received&x='+Date.now(),{cache:'no-store'}),fetch('/api/info?x='+Date.now(),{cache:'no-store'})]);let failed=rs.find(r=>!r.ok);if(failed){if(failed.status===401){showGate('Session expired — enter the current 6-digit PIN');return}throw new Error('HTTP '+failed.status)}let [a,b,i]=await Promise.all(rs.map(r=>r.json()));render($('sent'),a,'app');render($('received'),b,'received');$('stats').textContent='Used '+i.used_bytes+' bytes • Free '+i.free_bytes+' bytes'}catch(e){$('stats').textContent='Unable to refresh local server data ('+(e.message||'network error')+')'}}function esc(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}function render(g,a,b){files[b]=Array.isArray(a)?a:[];if(!files[b].length){g.innerHTML='<div class=muted>None</div>';return}g.innerHTML=files[b].map((p,i)=>{let u=esc(p.url),d=esc(p.download_url),n=esc(p.filename);return '<div class=card data-b='+b+' data-i='+i+'>'+(p.type=='image'?'<img class=thumb loading=lazy src="'+u+'" alt="'+n+'">':p.type=='video'?'<video class=videoThumb muted preload=metadata src="'+u+'"></video>':'<div style="aspect-ratio:1;display:grid;place-items:center;font-size:42px">📄</div>')+'<div class=meta>'+n+'<br><a class=action href="'+d+'" download>Download</a></div></div>'}).join('');g.querySelectorAll('.card').forEach(c=>c.onclick=e=>{if(e.target.closest('a'))return;open(files[c.dataset.b][c.dataset.i])})}function open(p){$('modal').classList.remove('hidden');let v=$('view'),e;if(p.type=='image'){e=document.createElement('img');e.src=p.url;e.dataset.s='1';e.style.transform='scale(1)'}else if(p.type=='video'){e=document.createElement('video');e.src=p.url;e.controls=true;e.autoplay=true}else{e=document.createElement('iframe');e.src=p.url;e.style='width:95vw;height:85vh;border:0;background:#fff'}v.innerHTML='';v.appendChild(e);$('dl').href=p.download_url;$('dl').download=p.filename;$('zi').onclick=()=>{if(p.type=='image'){e.dataset.s=Number(e.dataset.s)+.25;e.style.transform='scale('+e.dataset.s+')'}};$('zo').onclick=()=>{if(p.type=='image'){e.dataset.s=Math.max(.25,Number(e.dataset.s)-.25);e.style.transform='scale('+e.dataset.s+')'}};$('zr').onclick=()=>{if(p.type=='image'){e.dataset.s='1';e.style.transform='scale(1)'}}}$('cl').onclick=()=>{$('modal').classList.add('hidden');$('view').innerHTML=''};$('pair').onclick=pair;$('pin').onkeydown=e=>{if(e.key=='Enter')pair()};$('logout').onclick=()=>fetch('/api/logout',{method:'POST'}).finally(()=>location.reload());$('pick').onchange=()=>{[...$('pick').files].forEach(f=>{let x=new XMLHttpRequest();x.open('POST','/upload?source=web&filename='+encodeURIComponent(f.name));x.onload=()=>{if(x.status>=200&&x.status<300)load();else $('stats').textContent='Upload failed: '+f.name};x.onerror=()=>{$('stats').textContent='Upload failed: '+f.name};x.send(f)});$('pick').value=''};function autoText(){let e=$('textInput');e.style.height='auto';e.style.height=Math.min(e.scrollHeight,260)+'px';e.style.overflowY=e.scrollHeight>260?'auto':'hidden'}$('textInput').addEventListener('input',autoText);$('pasteText').onclick=async()=>{try{$('textInput').value=await navigator.clipboard.readText();autoText();$('textMsg').textContent='Pasted ✓'}catch(e){$('textMsg').textContent='Clipboard permission denied — paste manually'}};$('clearText').onclick=()=>{$('textInput').value='';autoText();$('textMsg').textContent=''};$('sendText').onclick=async()=>{let t=$('textInput').value;if(!t.trim()){$('textMsg').textContent='Enter some text first';return}$('sendText').disabled=true;$('textMsg').textContent='Sending...';try{let r=await fetch('/text',{method:'POST',headers:{'Content-Type':'text/plain; charset=utf-8'},body:t});let raw=await r.text();let d={};try{d=raw?JSON.parse(raw):{}}catch(_){}if(!r.ok)throw new Error(d.detail||raw||'Send failed');$('textInput').value='';autoText();$('textMsg').textContent='Sent to phone ✓';await load()}catch(e){$('textMsg').textContent='Failed: '+(e.message||'network error')}finally{$('sendText').disabled=false}};$('receiveText').onclick=async()=>{try{let r=await fetch('/files?source=received&x='+Date.now(),{cache:'no-store'});let raw=await r.text();if(!r.ok)throw new Error('HTTP '+r.status);let a=raw?JSON.parse(raw):[];let texts=a.filter(x=>(x.filename||'').toLowerCase().endsWith('.txt'));$('textReceived').innerHTML=texts.length?texts.map(x=>'<div style="padding:6px 0"><a class=action href="'+esc(x.url)+'" download>'+esc(x.filename)+'</a></div>').join(''):'No received text yet'}catch(e){$('textReceived').textContent='Unable to receive: '+(e.message||'error')}};fetch('/api/session',{cache:'no-store'}).then(r=>r.json()).then(d=>{if(d.authorized){$('gate').classList.add('hidden');$('app').classList.remove('hidden');load()}}).catch(()=>{});setInterval(()=>{if(!$('app').classList.contains('hidden'))load()},3000)</script></body></html>"""
    }
}