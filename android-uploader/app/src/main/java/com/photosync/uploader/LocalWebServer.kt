...
el('pair').onclick=()=>pair();el('pairTop').onclick=()=>pair(el('pinTop').value);el('pin').onkeydown=e=>{if(e.key==='Enter')pair()};el('copy').onclick=async()=>{try{await navigator.clipboard.writeText(location.origin);el('textMsg').textContent='Server URL copied ✓'}catch(e){el('textMsg').textContent='Copy failed'}};el('qr').onclick=showQr;
...
function render(id,a){let g=el(id);if(!a.length){g.innerHTML='<div class=empty style="grid-column:1/-1;min-height:120px">No files yet.</div>';return}g.innerHTML=a.slice(0,8).map(p=>{let v=p.type==='image'?'<img src="'+esc(p.url)+'" alt="">':p.type==='video'?'<video src="'+esc(p.url)+'" controls preload="metadata"></video>':p.type==='pdf'?'<div class=fileIcon>PDF</div>':'<div class=fileIcon>📄</div>';return '<div class=file onclick="openViewer(\\''+esc(p.url)+'\\',\\''+esc(p.type)+'\\',\\''+esc(p.filename)+'\\')">'+v+'<div class=meta>'+esc(p.filename)+'<br><span>'+formatBytes(p.size)+'</span><br><a href="'+esc(p.download_url)+'" download onclick="event.stopPropagation()">Download</a></div></div>'}).join('')}
...
async function copyText(v){try{await navigator.clipboard.writeText(v);el('textMsg').textContent='Copied ✓'}catch(e){el('textMsg').textContent='Copy failed'}}
function navigate(target){document.querySelectorAll('.nav div').forEach(x=>x.classList.remove('active'));let item=[...document.querySelectorAll('.nav div')].find(x=>x.dataset.target===target);if(item)item.classList.add('active');if(target==='send')el('pick').click();else if(target==='receive')el('received').scrollIntoView({behavior:'smooth'});else if(target==='text')el('textPanel').scrollIntoView({behavior:'smooth'});else if(target==='clients')el('clients').scrollIntoView({behavior:'smooth'});else if(target==='settings')showSettings();else window.scrollTo({top:0,behavior:'smooth'})}
el('sendFiles').onclick=()=>el('pick').click();el('receiveFiles').onclick=()=>navigate('receive');el('sendTextAction').onclick=()=>navigate('text');
