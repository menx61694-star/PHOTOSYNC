...
class AccountActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        setContentView(root)
        title = TextView(this).apply { text = "Create PhotoSync account"; textSize = 26f; setTextColor(Color.WHITE); setPadding(0, 0, 0, dp(18)) }
        root.addView(title)
        ...
        root.addView(name); root.addView(mobile); root.addView(username); root.addView(email); root.addView(password)
        action = Button(this).apply { text = "Create Account"; isAllCaps = false; setOnClickListener { submit() } }
        root.addView(action)
        root.addView(Button(this).apply { text = "Already have an account? Log in"; isAllCaps = false; setOnClickListener { signup = false; refreshMode() } })
        root.addView(Button(this).apply { text = "Need an account? Sign up"; isAllCaps = false; setOnClickListener { signup = true; refreshMode() } })
        refreshMode()
    }
    ...
    private fun submit() {
        ...
        if (emailText.isBlank() || pass.length < 8) { Toast.makeText(this, "Enter email and an 8+ character password", Toast.LENGTH_SHORT).show(); return }
        if (signup && (name.text.toString().trim().isBlank() || mobile.text.toString().trim().isBlank() || username.text.toString().trim().isBlank())) { Toast.makeText(this, "Name, mobile and username are mandatory", Toast.LENGTH_SHORT).show(); return }
        Thread {
            try {
                val form = FormBody.Builder().add("email", emailText).add("password", pass)
                if (signup) form.add("name", name.text.toString().trim()).add("mobile", mobile.text.toString().trim()).add("username", username.text.toString().trim())
                ...
                prefs.edit().putString("account_name", account?.optString("name", "") ?: "").putString("account_username", account?.optString("username", "") ?: "").putString("account_email", account?.optString("email", emailText) ?: emailText).apply()
                runOnUiThread { Toast.makeText(this, if (signup) "Account created ✓" else "Login successful ✓", Toast.LENGTH_SHORT).show(); refreshMode() }
            } catch (e: Exception) {
