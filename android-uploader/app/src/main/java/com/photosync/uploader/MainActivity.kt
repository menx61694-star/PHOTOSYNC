        ...
        socket = client.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                try {
                    ...
                    runOnUiThread {
                        if (source == "app") addFile(JSONObject(item), sentFilesContainer, "No files sent from this app yet")
                        else if (source == "web") addFile(JSONObject(item), receivedFilesContainer, "No files received from web yet")
                    }
                } catch (_: Exception) { }
            }
