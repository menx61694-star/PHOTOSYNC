...
// Build trigger: LAN discovery + live receive/preview fix
...
class MainActivity : AppCompatActivity() {
        ...
        socket = client.newWebSocket(Request.Builder().url(wsUrl()).build(), object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                try {
                    ...
                    runOnUiThread {
                        // Web uploads are received files. Unknown source is also treated as received
                        // so a missing/older source field cannot make a live file disappear.
                        if (source == "app") {
                        ...
                        // Re-read the server once after the event. This guarantees the UI stays in
                        // sync even if the event arrived before the file metadata was fully visible.
                        handler.postDelayed({ if (started) refreshLists() }, 250)
                    }
                } catch (_: Exception) {
                    // Ignore malformed WebSocket messages without breaking the live connection.
                }
            }
    ...
    /**
     * Finds broadcast addresses from the actual active IPv4 interfaces.
     * This works for normal Wi-Fi and for a phone acting as a hotspot;
     * it does not depend on WifiManager.dhcpInfo.
     */
