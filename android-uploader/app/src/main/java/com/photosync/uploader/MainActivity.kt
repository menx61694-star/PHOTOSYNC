...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() { super.onStart(); started = true; applyThemeColor(); discoverServer() }
    ...
    private fun applyThemeColor() {
        ...
        try {
            ...
            if (::status.isInitialized) status.setTextColor(Color.WHITE)
    ...
    private fun connectSocket() {
        ...
        socket = client.newWebSocket(Request.Builder().url(wsUrl()).build(), object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                try {
                    val data = JSONObject(text); if (data.optString("type") != "file_uploaded") return
                    val source = data.optString("source", "unknown")
                    runOnUiThread {
                        if (source == "app") addFile(data, sentFilesContainer, "No files sent from this app yet") else addFile(data, receivedFilesContainer, "No files received from web yet")
                        handler.postDelayed({ if (started) refreshLists() }, 250)
                    }
                } catch (_: Exception) {}
            }
        ...
        try {
            ...
            while (interfaces.hasMoreElements()) {
                ...
                for (interfaceAddress in networkInterface.interfaceAddresses) {
                    val address = interfaceAddress.address; val broadcast = interfaceAddress.broadcast
    ...
    private fun createFileRow(item: JSONObject): LinearLayout {
        val name = item.optString("filename", "file"); val url = item.optString("url", ""); val size = item.optLong("size", 0L); val type = item.optString("type", "file"); val mime = item.optString("content_type", "application/octet-stream")
        val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(8), dp(6), dp(8), dp(6)); setOnClickListener { if (type == "image") showImagePreview(url, name, mime) else downloadFile(url, name, mime) } }
        if (type == "image") { val image = ImageView(this).apply { layoutParams = LinearLayout.LayoutParams(dp(72), dp(72)); scaleType = ImageView.ScaleType.CENTER_CROP; setBackgroundColor(0xFF2B2B2B.toInt()); contentDescription = name }; row.addView(image); loadThumbnail(currentServerUrl() + url, image) }
    ...
    private fun loadThumbnail(url: String, image: ImageView) { Thread { try { val request = Request.Builder().url(url).build(); client.newCall(request).execute().use { response -> if (!response.isSuccessful) return@use; val bytes = response.body?.bytes() ?: return@use; val bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size, BitmapFactory.Options().apply { inSampleSize = 4 }); if (bitmap != null) runOnUiThread { image.setImageBitmap(bitmap) } } } catch (_: Exception) {} }.start() }
    private fun showImagePreview(path: String, name: String, mime: String) {
        val base = currentServerUrl(); if (base.isBlank() || path.isBlank()) return; val fullUrl = if (path.startsWith("http://") || path.startsWith("https://")) path else "$base${if (path.startsWith("/")) path else "/$path"}"
        ...
        val dialog = AlertDialog.Builder(this).setTitle(name).setView(imageView).setNegativeButton("Close", null).setPositiveButton("Download") { _, _ -> downloadFile(path, name, mime) }.create(); dialog.show()
        Thread { try { val request = Request.Builder().url(fullUrl).get().build(); client.newCall(request).execute().use { response -> if (!response.isSuccessful) return@use; val bytes = response.body?.bytes() ?: return@use; val bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size, BitmapFactory.Options()); if (bitmap != null) runOnUiThread { if (dialog.isShowing) imageView.setImageBitmap(bitmap) } } } catch (_: Exception) {} }.start()
    }
    ...
    private fun downloadFile(path: String, name: String, mime: String) {
        val base = currentServerUrl(); if (base.isBlank() || path.isBlank()) { Toast.makeText(this, "Server not connected", Toast.LENGTH_SHORT).show(); return }; val fullUrl = if (path.startsWith("http://") || path.startsWith("https://")) path else "$base${if (path.startsWith("/")) path else "/$path"}"
        Thread { try { val request = Request.Builder().url(fullUrl).get().build(); client.newCall(request).execute().use { response -> if (!response.isSuccessful) error("HTTP ${response.code}"); val body = response.body ?: error("Empty file"); val values = ContentValues().apply { put(MediaStore.Downloads.DISPLAY_NAME, name); put(MediaStore.Downloads.MIME_TYPE, mime.ifBlank { "application/octet-stream" }); put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS); put(MediaStore.Downloads.IS_PENDING, 1) }; val uri = contentResolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values) ?: error("Cannot create download"); try { contentResolver.openOutputStream(uri).use { output -> requireNotNull(output) { "Cannot open download" }; body.byteStream().use { input -> input.copyTo(output!!) } }; contentResolver.update(uri, ContentValues().apply { put(MediaStore.Downloads.IS_PENDING, 0) }, null, null); runOnUiThread { Toast.makeText(this, "Downloaded: $name", Toast.LENGTH_SHORT).show() } } catch (e: Exception) { contentResolver.delete(uri, null, null); throw e } } } catch (e: Exception) { runOnUiThread { Toast.makeText(this, "Download failed: ${e.message}", Toast.LENGTH_LONG).show() } } }.start()
    }
}
